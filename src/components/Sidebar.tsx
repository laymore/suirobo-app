...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  return (
    <div style={{
      ...
      {/* ── Chat History (AI Assistant) — removed: the product is Autobots, not an
             AI chat agent. Kept mounted but hidden so session plumbing stays intact. ── */}
      <div style={{ display: 'none', flexDirection: 'column', gap: 8, marginTop: 24, flex: 1, overflowY: 'auto' }}>
      ...
      {/* push the desktop CTA to the bottom (chat history is hidden now) */}
      <div style={{ flex: 1 }} />

      {/* ── Get the full desktop app (web only) ──
          The web app is wallet-signed (Manual / Web Bot). Fully hands-off 24/7
          automation lives in the downloaded desktop app, where the key is entered
          locally. No "connect to a running local agent" on the web anymore. */}
      {!isDesktop && (
        <div style={{ flex: 'none', marginTop: 16 }}>
          ...
          <div style={{ fontSize: '0.66rem', color: '#64748b', textAlign: 'center', margin: '8px 0 0', lineHeight: 1.5 }}>
            Run Auto Bots 24/7 with your key on your own machine.
            {latestVersion && <> · <span style={{ fontFamily: 'monospace', color: '#94a3b8' }}>v{latestVersion}</span></>}
          </div>
        </div>
      )}
