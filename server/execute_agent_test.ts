...
const allTools = [...marginTools, ...predictTools, ...deepbookV3Tools];
...
async function testCommands() {
  ...
  let currentGasCoin: { objectId: string, version: string, digest: string } | null = null;
  console.log("=================================================");
  console.log("🚀 Bắt đầu test các lệnh Spot (DeepBook V3) với Agent DeepTrade");
  console.log(`🔑 Ví mainnet: ${address}`);
  console.log("=================================================\n");

  const commands = [
    `Địa chỉ ví của tôi là: ${address}. Hãy thực hiện chuỗi thao tác Margin tự động sau: 1) Nạp 1 SUI vào Margin Pool SUI_USDC. 2) Kiểm tra sức khỏe/số lượng tài sản trong Margin Pool. 3) Mở vị thế Margin bằng cách vay 1 USDC từ pool. 4) Đóng vị thế đó bằng cách trả lại khoản vay 1 USDC. 5) Rút lại 1 SUI ra khỏi pool. Chạy chế độ autonomous cho tất cả thao tác.`
  ];
