...
async function testCommands() {
  ...
  const suiClient = new SuiJsonRpcClient({ url: 'https://fullnode.mainnet.sui.io' });
  let currentGasCoin: any = null;

  console.log("=================================================");
  console.log("🚀 Bắt đầu test Margin (DeepBook V3) trên MAINNET");
  console.log("🔑 Ví testnet:", address);
  console.log("=================================================\n");

  const commands = [
    `Địa chỉ ví của tôi là: ${address}. Lệnh yêu cầu: 1. Hãy dùng Tool margin_analyzer_skill để phân tích trạng thái Margin cho SUI. 2. Sau khi có báo cáo JSON, nếu thấy an toàn, hãy mở MỘT vị thế Margin Long. (Sử dụng Tool margin_open_position). Dùng 0.1 SUI làm collateral, mượn 0.1 USDC. Chạy ở chế độ autonomous.`
  ];
