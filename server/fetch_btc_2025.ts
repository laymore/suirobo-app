/**
 * Fetch REAL Binance BTCUSDT klines for FULL YEAR 2025 (UTC) in 4 timeframes
...
async function fetchTf(binanceTf: string): Promise<Candle[]> {
  ...
  let cursor = START;
  while (cursor < END) {
    const url = `https://api.binance.com/api/v3/klines?symbol=BTCUSDT&interval=${binanceTf}&startTime=${cursor}&endTime=${END - 1}&limit=1000`;
...
async function main() {
  ...
  for (const tf of TFS) {
    ...
    const pub = path.join(process.cwd(), 'public', 'data', `btc_full_2025_${tf.tag}.json`);
    const srv = path.join(process.cwd(), 'server', 'data', `btc_full_2025_${tf.tag}.json`);
